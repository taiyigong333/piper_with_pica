...
- 所有设备适配器输出 `RobotState`：关节为 `rad`，位置为 `m`；TCP 由 `session.pose_representation` 标注为 `xyz_rxryrz`（`m + rad`）或 `xyz_xyzw`。
- Piper 反馈关节为 `0.001°`，适配器乘以 `pi / 180000`；位置为 `0.001 mm`，适配器乘以 `1e-6`。
- Piper 夹爪通过同一只读 CAN 连接调用 `GetArmGripperMsgs()`；其 `grippers_angle` 为 `0.001 mm` 行程，适配器乘以 `1e-6` 后以 `m` 写入 `end_effector_single_position_*`。适配器会等待首个 `0x2A8` 反馈，拒绝 SDK 尚未收到数据时的默认零值；该读取不会下发夹爪控制帧。
- Piper 默认将末端反馈的 `[RX, RY, RZ]` 由 `0.001°` 转为 `rad` 后，以原生 `xyz_rxryrz` 写入。计算 `tool_offset_m` 时才按 SDK 示例的 `Rz * Ry * Rx` 约定临时转换为四元数；`xyz_xyzw` 仍可通过配置用于兼容旧数据。该旋转顺序仍应通过现场标定验证。

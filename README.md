...
完整命令说明见 [docs/使用说明.md](docs/使用说明.md)，查看器操作见 [docs/2026-07-20_02_轨迹可视化查看器.md](docs/2026-07-20_02_轨迹可视化查看器.md) 和 [docs/2026-07-21_01_轨迹查看器时序细节.md](docs/2026-07-21_01_轨迹查看器时序细节.md)。
...
会话所有阶段均按单个空格推进：校准完成、预检后的安全确认、Sense 双击并实际跟随、以及实体设备停止遥操之后。`--repeat` 仅在第一条要求确认校准，后续仍会完整重启预检、起始位姿、遥操和两路 RViz。每条完成后按空格保留、按 `d` 删除、按 `q` 保留，不再输入 `yes/no`；传入 `--on-complete save/delete` 可跳过此交互。

现场执行顺序为：先使用 `teleop-status` 确认没有旧 Pika 进程，再校准、预检、读取并填写起始位姿，最后启动会话。动作结束时第一次空格会立即禁止新帧入队，再完成写盘；确认写盘完成后，双击 Sense 停止遥操并按第二次空格关闭 ROS，最后按空格保留或按 `d` 删除本条轨迹。关闭时控制器首轮只中断其 `ros2 launch` 父进程，传感器首轮中断进程组，终端会输出各阶段等待时间。`LIBUSB_ERROR_BUSY (-6)` 表示 Sense USB 接口仍被其他进程占用，不能将该次校准视为可靠完成。编排层启动 ROS 前会移除 uv 与 OpenCV 的 Python/Qt 环境变量，并检查包括两路 `rviz2` 在内的关键节点日志；`pika_ros` 的代码和 launch 文件不会被修改。各脚本职责、配置示例、完整操作顺序、残留进程清理与轨迹检查见 [docs/2026-07-19_01_遥操脚本与现场操作说明.md](docs/2026-07-19_01_遥操脚本与现场操作说明.md)、[docs/2026-07-20_01_遥操收尾与轨迹检查.md](docs/2026-07-20_01_遥操收尾与轨迹检查.md)、[docs/2026-07-20_03_重复遥操响应与RViz.md](docs/2026-07-20_03_重复遥操响应与RViz.md)、[docs/2026-07-20_04_轨迹保留与删除.md](docs/2026-07-20_04_轨迹保留与删除.md) 和 [docs/2026-07-20_05_ROS收尾优化试验.md](docs/2026-07-20_05_ROS收尾优化试验.md)。

...
现场执行顺序为：先使用 `teleop-status` 确认没有旧 Pika 进程，再校准、预检、读取并填写起始位姿，最后启动会话。动作结束时第一次空格只结束采集和写盘；确认写盘完成后，双击 Sense 停止遥操并按第二次空格关闭 ROS。`LIBUSB_ERROR_BUSY (-6)` 表示 Sense USB 接口仍被其他进程占用，不能将该次校准视为可靠完成。编排层启动 ROS 前会移除 uv 的 Python 环境变量，并检查关键 ROS 节点日志；`pika_ros` 的代码和 launch 文件不会被修改。各脚本职责、配置示例、完整操作顺序、残留进程清理与轨迹检查见 [docs/2026-07-19_01_遥操脚本与现场操作说明.md](docs/2026-07-19_01_遥操脚本与现场操作说明.md) 和 [docs/2026-07-20_01_遥操收尾与轨迹检查.md](docs/2026-07-20_01_遥操收尾与轨迹检查.md)。
